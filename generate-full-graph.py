...
if __name__ == '__main__':
    train_results = torch.load("scripts/text-results-bert-full-train-4.pickle")
    eval_results = torch.load("scripts/text-results-bert-full-eval-4.pickle")
    ...
    inputs = torch.load("scripts/text-result-gt-full-eval-4.pickle")
    ...
    for i in keys:
        ...
        for j in range(4):
            ...
            s = x[2].split(".")[j]
            print("\t", s)
            print("\t\texpected KW : ", " ".join(x[3]))
            print("\t\texpected    : ", x[7])
            print("\t\tgenerated KW: ", " ".join(x[4]))
            print("\t\tgenerated   : ", x[5])
        print()
