...
def make_indiviual_plot(ax0):
    ...
    #for i in keys:
    #    ax0.axvline(x=i, color='red', alpha=0.5)
    ax0.legend()


if __name__ == '__main__':
    train_results = torch.load("scripts/keywords-results-f1-train-4.pickle")
    eval_results = torch.load("scripts/keywords-results-f1-eval-4.pickle")
    ...
    inputs = torch.load("scripts/keywords-result-eval-4.pickle")
    ...
    for i in keys:
        ...
        for j in range(4):
            ...
            s = x[2].split(".")[j]
            print("\t", s)
            print("\t\texpected:  ", " ".join(x[3]))
            print("\t\tgenerated: ", " ".join(x[4]))
        print()
